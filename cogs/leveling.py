...
LevelUPresponses = [
    f"Congrats {LevelUpAnnouncement.Member.mention}! You are now level {LevelUpAnnouncement.LEVEL} 😎",
...
class Leveling(commands.Cog):
    ...
    async def make_xp_wipe_onleave_toggle(self, ctx):
        async with aiosqlite.connect("./databases/leveling_wipe_toggle.db") as db:
            await db.execute(
                "CREATE TABLE IF NOT EXISTS xp_wipe_onleave (guild_id TEXT, status int DEFAULT 0)"
            )
            await db.commit()
            await ctx.send("Done")
            for guild in self.client.guilds:
                await db.execute(
                    "INSERT OR IGNORE INTO xp_wipe_onleave (guild_id) VALUES (?)",
                    (str(guild.id),),
                )
    ...
    @commands.command(
        name="xp_wipe_onleave_toggle",
        help="This command toggles the xp wipe on leave feature.",
    )
    @commands.has_permissions(administrator=True)
    async def xp_wipe_onleave_toggle_command(self, ctx):
        async with aiosqlite.connect("./databases/leveling_wipe_toggle.db") as db:
            status = await db.execute(
                "SELECT status FROM xp_wipe_onleave WHERE guild_id = ?",
                (str(ctx.guild.id),),
            )
            status = await status.fetchone()
            if status[0] == 0:
                await db.execute(
                    "UPDATE xp_wipe_onleave SET status = 1 WHERE guild_id = ?",
                    (str(ctx.guild.id),),
                )
                await db.commit()
                ...
                await db.execute(
                    "UPDATE xp_wipe_onleave SET status = 0 WHERE guild_id = ?",
                    (str(ctx.guild.id),),
                )
                await db.commit()
    ...
    @commands.slash_command(
        name="xp_wipe_onleave_toggle",
        description="This command toggles the xp wipe on leave feature.",
    )
    @commands.has_permissions(administrator=True)
    ...
    @commands.command(
        aliases=["lvl"],
        help="This command shows your rank for the leveling system.",
        description="Shows your rank image",
    )
    async def rank(self, ctx, member: discord.Member = None):
        ...
        try:
            ...
            profile_image = await load_image_async(
                str("https://cdn.discordapp.com/embed/avatars/0.png")
            )
    ...
    @commands.command(
        aliases=["lb"],
        help="This command shows the leaderboard for this server.\nIt is sorted by most highest level to lowest.",
    )
    ...
    # reset users xp to 0
    @commands.command(
        aliases=["reset"], help="This command resets the xp of a user to 0."
    )
    @commands.has_permissions(administrator=True)
    ...
    @commands.command(
        aliases=["resetall"], help="This command resets all users xp to 0."
    )
    @commands.has_permissions(administrator=True)
    ...
    async def addxpignore(self, ctx, channel: discord.TextChannel):
        async with aiosqlite.connect("./databases/xp_ignore.db") as db:
            await db.execute(
                "INSERT INTO xp_ignore VALUES (?, ?)", (ctx.guild.id, channel.id)
            )
            await db.commit()
    ...
    async def removexpignore(self, ctx, channel: discord.TextChannel):
        async with aiosqlite.connect("./databases/xp_ignore.db") as db:
            try:
                await db.execute(
                    "DELETE FROM xp_ignore WHERE channel_id = ?", (channel.id,)
                )
                await db.commit()
    ...
    async def listxpignore(self, ctx):
        async with aiosqlite.connect("./databases/xp_ignore.db") as db:
            data = await db.execute(
                "SELECT * FROM xp_ignore WHERE guild_id = ?", (ctx.guild.id,)
            )
            data = await data.fetchall()
            if data:
                await ctx.send(
                    f"List of channels in the xp ignore list: {', '.join([f'<#{channel[1]}>' for channel in data])}"
                )
            else:
