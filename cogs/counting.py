...
async def counting(msg, guild, channel, m):
    ...
    if channel.id == cc:
        ...
        for i in dataa:
            if i['guild_id'] == guild.id:
                if i['lastcounter'] == None:
                    ...
                    em = discord.Embed(title=f"{m.author.display_name}, You ruined it!", description="Only one person at a time\nCount reset to zero")
        ...
        if (data[f"{guild.id}"] + 1) == msg:
            ...
            await m.add_reaction("❌")
            em = discord.Embed(title=f"{m.author.display_name}, You ruined it!", description=f"You were supposed to type `{(data[f'{guild.id}']+1)}`\nCount reset to zero")
            i['lastcounter'] = None
            data[f"{guild.id}"] = 0
