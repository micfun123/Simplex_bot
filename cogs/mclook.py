...
class Minecraft(commands.Cog):
    ...
    @commands.command(
        name="mcping",
        aliases=["mcstatus"],
        help="Gets the status of a Minecraft server, Ping and player count",
    )
    async def mc_ping(self, ctx, server: str):
        async with ctx.typing():
            ...
            try:
                ...
                embed = discord.Embed(
                    title=f"{server} is online", description=MOTD, colour=0x00FF00
                )
                try:
                    mods = resp["mods"]
                ...
                if players > 20:
                    embed.add_field(
                        name="Player list: ",
                        value="Too many players to display",
                        inline=False,
                    )
                elif players == 0:
                    pass
                    ...
                    for player in playerlist:
                        embed.add_field(
                            name="Player list: ", value=player, inline=False
                        )
    ...
    @commands.slash_command(
        name="mcping",
        description="Gets the status of a Minecraft server, Ping and player count",
    )
    async def mc_ping__slash(self, ctx, server: str):
        ...
        try:
            ...
            try:
                ...
                embed.add_field(name="Is modded: ", value="False", inline=True)
            embed.add_field(
                name="Players Online: ", value=f"{players} / {playersmax}", inline=True
            )
            embed.add_field(name="Version: ", value=version, inline=True)
            if players > 20:
                embed.add_field(
    ...
    @commands.slash_command(
        name="mcskin", description="Gets a skin of a minecraft player"
    )
    ...
    @commands.slash_command(
        name="mchead", description="Gets a skin of a minecraft player"
    )
