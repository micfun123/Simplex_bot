...
class QOTD(commands.Cog):
    ...
    async def setqotd(self, ctx, channel: discord.TextChannel):
        ...
        if len(data) == 0:
            ...
            cur.execute(
                "UPDATE qotd SET channel_id = ? WHERE server_id = ?",
    ...
    async def disableqotd(self, ctx):
        ...
        if (
            len(
                cur.execute(
                    "SELECT * FROM qotd WHERE server_id = ?", (ctx.guild.id,)
                ).fetchall()
            )
            == 0
        ):
    ...
    async def qotd(self):
        ...
        for server in cur.execute("SELECT server_id, channel_id FROM qotd").fetchall():
            try:
                ...
                embed = discord.Embed(
                    title="QOTD", description=question, color=0x00FF00
                )
                embed.add_field(
                    name="options",
                    value=f"{options[0]} \n {options[1]} \n {options[2]} \n {options[3]}",
    ...
    async def qotdtest(self, ctx):
        ...
        for server in cur.execute("SELECT server_id, channel_id FROM qotd").fetchall():
            try:
                ...
                embed.add_field(
                    name="Answer for yesterdays QOTF is",
                    value=f"||{yesterday_answer}||",
                    inline=False,
                )
                embed.set_footer(
