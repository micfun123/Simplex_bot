...
class BattleshipsPlayer(Player):
    def __init__(self, discord_id: int):
        ...
        self.visible_fleet: list[Tiles] = [
            Water()
        ] * 100  # this is the fleet that the other player will be able to see
    ...
    def build_fleet(self):
        ...
        def place(ship_to_be_placed: Ship):
            while True:
                ...
                for _ in range(
                    ship.size - 1
                ):  # the -1 represents the core that has already been placed
                ...
                if (
                    len([x for x in fleet_copy if isinstance(x, Ship)])
                    == expected_ships
                ):  # no ship collision occurred
                    return fleet_copy
...
class BattleshipsGame(Game):
    ...
    def __init__(self, players: list[int]):
        ...
        self.players = [
            BattleshipsPlayer(self.player_ids[0]),
            BattleshipsPlayer(self.player_ids[1]),
        ]
    ...
    def next_player(self):
        return (
            self.players[0]
            if self.current_round_player != self.players[0]
            else self.players[1]
        )
    ...
    def display(
        self, discord_id: [int, None] = None, view_opponent_fleet: bool = True
    ) -> str:
    ...
    def get_player_by_id(self, discord_id: int) -> BattleshipsPlayer:
        return (
            self.players[0]
            if discord_id == self.players[0].discord_id
            else self.players[1]
        )
