...
class TicTacToeGame(Game):
    TIMEOUT = (
        100  # if you change this make sure to change it in the tchallenge command to
    )
    ...
    def next_player(self):
        return (
            self.players[0]
            if self.players[0] != self.current_round_player
            else self.players[1]
        )
    ...
    def get_player_by_id(self, discord_id: int):
        return (
            self.players[0]
            if self.players[0].discord_id == discord_id
            else self.players[1]
        )
