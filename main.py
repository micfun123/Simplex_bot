...
import aiosqlite
import urllib.parse
from tools.get_channel_info import get_announcement_channel , set_announcement_channel_tool, get_counting_channel
...
app.config["DISCORD_CLIENT_ID"] = os.getenv("client_id") # Your Client ID here   # Discord client ID.
...
async def dashboard_server(guild_id):
	...
	announment_channel_name = await ipc_client.request("get_channel_name", guild_id = guild_id, channel_id = announment_channel)
	counting_channel = await get_counting_channel(guild_id)
	counting_channel_name = await ipc_client.request("get_channel_name", guild_id = guild_id, channel_id = counting_channel)
	

	name = (await discord.fetch_user()).name
	return await render_template("dashboard_server.html", guild_id = guild_id, servername = servername, prefix = prefix, channels = channels,name = name,announment_channel = announment_channel_name, counting_channel = counting_channel_name)
...
async def set_counting_channel(guild_id):
	...
	async with aiosqlite.connect("./databases/counting.db") as db:
		data = await db.execute("SELECT * FROM counting WHERE guild_id = ?", (guild_id,))
		data = await data.fetchall()
		if len(data) == 0:
			if channel_id == "None":
				channel_id = None
			await db.execute("INSERT INTO counting VALUES (?, ?, ?, ?, ?,?)", (guild_id, channel_id, 0, 0, None, 0))
			await db.commit()
		else:
			if channel_id == "None":
				channel_id = None
			await db.execute("UPDATE counting SET counting_channel = ? WHERE guild_id = ?", (channel_id, guild_id))
			await db.commit()
	return redirect(url_for("dashboard_server", guild_id = guild_id))
