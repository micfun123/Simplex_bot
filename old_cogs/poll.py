...
class Polls(commands.Cog):
    ...
    async def quickpoll__command(self, ctx, *questions_and_choices: str):
        ...
        em = discord.Embed(
            title=question,
            description="\n".join(
                f"{keycap}: {content}" for keycap, content in choices
            ),
            color=0x20BEFF,
        )
        em.set_footer(text=f"Poll created by {ctx.author}")
    ...
    async def quickpoll__slash(
        ...
        try:
            ...
            for emoji, _ in choices:
                await poll.add_reaction(emoji)
            await ctx.respond(
                "Poll created!\n If you like this bot a vote would be appreciate https://top.gg/bot/902240397273743361",
                ephemeral=True,
            )
    ...
    async def quickpoll__slash_error(self, ctx, error):
        if isinstance(error, commands.BotMissingPermissions):
            return await ctx.respond(
                "I don't have permissions to add reactions", ephemeral=True
            )
        if isinstance(error, commands.MissingPermissions):
            return await ctx.respond(
                "You don't have permissions to add reactions", ephemeral=True
            )
        if isinstance(error, discord.errors.Forbidden):
            return await ctx.respond(
                "I don't have permissions to post in that channel", ephemeral=True
            )
