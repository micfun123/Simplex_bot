...
class mastodon(commands.Cog):
    ...
    @commands.slash_command(
        name="mastodon_manage", description="Manage the mastodon feeds"
    )
    ...
    async def mastodon_manage(
        ...
        if action == "add":
            async with aiosqlite.connect("databases/mastodon.db") as db:
                await db.execute(
                    "INSERT INTO mastodon (channel_id, guild_id, username, last_posted) VALUES (?, ?, ?, ?)",
                    (channel.id, channel.guild.id, username, "0"),
                )
                await db.commit()
            ...
            async with aiosqlite.connect("databases/mastodon.db") as db:
                await db.execute(
                    "DELETE FROM mastodon WHERE channel_id = ? AND guild_id = ? AND username = ?",
                    (channel.id, channel.guild.id, username),
                )
                await db.commit()
            await ctx.respond("Done")
        elif action == "list":
            async with aiosqlite.connect("databases/mastodon.db") as db:
                cursor = await db.execute(
                    "SELECT * FROM mastodon WHERE guild_id = ?", (channel.guild.id,)
                )
                data = await cursor.fetchall()
                embed = discord.Embed(
                    title="Mastodon feeds",
                    description="Here are all the mastodon feeds for this server",
                    color=0x00FF00,
                )
                for row in data:
                    embed.add_field(
                        name=f"{row[2]}",
                        value=f"Channel: <#{row[0]}>\nUsername: {row[2]}\nLast posted: {row[3]}",
                        inline=False,
                    )
            ...
            await ctx.respond(
                "That is not a valid action, valid actions are: add, remove,list"
            )
        await ctx.respond(
            "Thank you for using simplex voting and donating helps keep this bot free.\n We do a check once every 10 minutes.",
            ephemeral=True,
        )
            ...
            for row in data:
                try:
                    ...
                    if int(last_post) != int(last_posted):
                        ...
                        await db.execute(
                            "UPDATE mastodon SET last_posted = ? WHERE username = ?",
                            (last_post, username),
                        )
                        await db.commit()
                    else:
